...
//                        $receiptPrice += !isset($inReceipts[$child->stock_id]) ? $child->price : 0;
        ...
        if (data_get($data, 'type') == Order::DELIVERY) {
            // Yandex Delivery
            $addressesDataForYaDelivery = ['userAddress' => $data["address"], 'shopAddress' => $cart->shop->location];
            $yandexDelivery = $this->yaDeliveryService->estimateCost($addressesDataForYaDelivery);

            //yandexDeliveryFee
            $deliveryFee = $yandexDelivery['price'];
            $km = round($yandexDelivery['distance_meters'] / 1000, 2);
        } else {
            $deliveryFee = 0;
            $km = 0;
        }
