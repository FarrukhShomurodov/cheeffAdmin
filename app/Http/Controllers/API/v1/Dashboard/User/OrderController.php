...
class OrderController extends UserBaseController
{
    ...
    protected OrderStatusUpdateService $orderStatusUpdateService;
     ...
     */
    public function __construct(OrderRepoInterface $orderRepository, OrderServiceInterface $orderService, YaDeliveryService $yaDeliveryService, OrderStatusUpdateService $orderStatusUpdateService)
    {
        parent::__construct();
    ...
    public function show(int $id): JsonResponse
    {
        ...
        if ($getOrder->status === "on_a_way") {
            $yaDeliveryUuid = $getOrder->yaDelivery()->first()->ya_delivery_uuid;
            $yandexDelivery = $this->yaDeliveryService->claimInfo($yaDeliveryUuid);

            if ($yandexDelivery['status'] == 'delivered'){
                $this->orderStatusUpdateService->statusUpdate($getOrder,'delivered');
            };

            if ($yandexDelivery['status'] == 'performer_not_found'){
                $this->orderStatusUpdateService->statusUpdate($getOrder,'ready');
            }


            // Update price
            if ($yandexDelivery && isset($yandexDelivery['pricing']['offer'])) {
                ...
                // Update the order's delivery fee and total price
                $getOrder->update([
