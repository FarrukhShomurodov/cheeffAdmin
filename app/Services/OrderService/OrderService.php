...
//
                ...
                if (data_get($data, 'delivery_type') == Order::DELIVERY) {
                    ...
                    $shopLocation = clone $order->fresh($this->with())->shop;
                    $userAddress = clone $order->fresh($this->with());

                    $yandexDeliveryEstimateCost = $this->yaDeliveryService->estimateCost(['shopAddress' => $shopLocation['location'], 'userAddress' => $userAddress['location']]);

                    $deliveryFee = intval($yandexDeliveryEstimateCost['price']);
    ...
    private function calculateOrder(Order $order, ?Shop $shop, array $data): void
    {
        ...
        if ($coupon?->for === 'delivery_fee') {
            ...
            $totalPrice -= max($this->checkCoupon($coupon, $order, $totalPrice - $order->delivery_fee), 0);
